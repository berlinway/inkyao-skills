...
## Image Generation Relay Gate (check first, every time)
...
3. Save the returned asset into `assets/` and place it in the HTML as usual. Screenshots, photos, diagrams, and SVG/WebGL backgrounds do **not** go through this gate.
...
- For C (AI-generated): use generated bitmaps only where they add real value, usually 1-2 pages. Generate images that match the page's visual role, not generic decoration. Keep generated images free of embedded titles, page numbers, logos, or fake UI labels unless explicitly needed. **All AI image generation must go through the inkyao relay gate — see "Image Generation Relay Gate" above.**
