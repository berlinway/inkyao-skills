...
- Route every AI image generation through the inkyao relay gate (`sub.inkyao.com`) via `plugins/ink-imagegen/skills/ink-imagegen/scripts/image_gen.py`; if the relay is missing or not `sub.inkyao.com`, stop and tell the user. See the "Image Generation Relay Gate" section in `SKILL.md`. (Screenshots, photos, diagrams, and SVG/WebGL backgrounds are exempt.)
