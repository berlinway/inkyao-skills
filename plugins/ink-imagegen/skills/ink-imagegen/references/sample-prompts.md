...
These prompt recipes are for the ink relay CLI used by this skill.
...
Execution details such as explicit CLI flags, `quality`, `input_fidelity`, masks, output formats, and local output paths are handled by the ink relay CLI. For true transparent images, use `gpt-image-1.5 --background transparent --output-format png` when the relay supports it. If native transparency is unavailable, prompt for a flat chroma-key background and remove it locally with `python "/Users/eliot/code/yb/niche/ink-imagegen/scripts/remove_chroma_key.py"`.

ink relay CLI model notes:
- `gpt-image-2` is the ink relay CLI default for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- If transparent output needs true native transparency, use `gpt-image-1.5` because `gpt-image-2` does not support `background=transparent`.
...
Post-process note: when using a chroma-key source instead of native transparency, run `python "/Users/eliot/code/yb/niche/ink-imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`.
