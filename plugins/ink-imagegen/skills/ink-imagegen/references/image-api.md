...
This file documents Image API parameters used by the ink relay CLI for generation, editing, batch generation, model controls, transparency, and output handling.

These parameters describe the Image API and bundled ink relay CLI surface.

## Scope
- This ink relay CLI is intended for GPT Image models (`gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, and `gpt-image-1-mini`).
- Route requests through the ink relay CLI, not Codex's built-in image tool.
...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter.

Use ink relay CLI `gpt-image-1.5` with `background=transparent` and a transparent-capable output format such as `png` or `webp` when true/native transparency is required and the relay supports it. If the relay rejects native transparency, generate a flat chroma-key background and remove it locally with `/Users/eliot/code/yb/niche/ink-imagegen/scripts/remove_chroma_key.py`.
...
- If a request fails because a specific option is unsupported by the selected GPT Image model, retry manually without that option only when the option is not required by the user. If true transparent output is required, switch to `gpt-image-1.5` instead of dropping `background=transparent` when the relay supports that model.

## Important boundary
- `quality`, `input_fidelity`, explicit masks, `background`, `output_format`, and related parameters are ink relay CLI execution controls.
