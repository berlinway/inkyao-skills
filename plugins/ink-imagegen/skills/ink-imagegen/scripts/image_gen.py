#!/usr/bin/env python3
"""Ink relay CLI for image generation or editing with GPT Image models.

This fork is intended for OpenAI-compatible relay endpoints. It keeps the full
imagegen CLI surface while routing requests through a configured base URL.
...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("OPENAI_API_KEY"):
        ...
        return
    if _codex_auth_api_key():
        print("OPENAI_API_KEY loaded from Codex auth config.", file=sys.stderr)
        return
    if dry_run:
        _warn("OPENAI_API_KEY is not set and was not found in Codex auth config; dry-run only.")
        return
    _die("OPENAI_API_KEY is not set and was not found in Codex auth config.")
...
def _codex_auth_api_key() -> Optional[str]:
    try:
        ...
        _warn(f"Could not read Codex auth config: {exc}")
        return None
...
def _codex_config_base_url() -> Optional[str]:
    try:
        data = tomllib.loads(CODEX_CONFIG_PATH.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    except Exception as exc:
        _warn(f"Could not read Codex config: {exc}")
        return None
...
def _api_key() -> Optional[str]:
    ...
    return _codex_auth_api_key()


def _relay_base_url() -> Optional[str]:
...
def _print_relay_config(dry_run: bool) -> None:
    base_url = _relay_base_url()
    if base_url:
        source = "environment" if any(os.getenv(name) for name in BASE_URL_ENV_VARS) else "Codex config"
        print(f"Using relay base URL from {source}: {base_url}", file=sys.stderr)
        return
    if dry_run:
        _warn(
            "No relay base URL set and none was found in Codex config; dry-run only. Set RELAY_IMAGE_BASE_URL, OPENAI_BASE_URL, OPENAI_API_BASE, or model_providers.<provider>.base_url."
        )
        return
    _die("Relay base URL is not set and none was found in Codex config.")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Ink relay CLI for image generation or editing via GPT Image models"
    )
