...
These prompting principles are for the ink relay CLI used by this skill.

This file is about prompt structure, specificity, and iteration. Ink relay CLI execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in the ink relay CLI docs.
...
- [Ink relay CLI execution controls](#relay-cli-execution-controls)
...
- In ink relay CLI mode, use `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Use model-native transparency through the relay when supported: `--model gpt-image-1.5 --background transparent --output-format png`.
- Do not use `--background transparent` with `gpt-image-2`; that model rejects true transparent output.
- If the relay/model does not support native transparency, prompt for a perfectly flat solid chroma-key background, usually `#00ff00`; use `#ff00ff` when the subject is green, and avoid key colors that appear in the subject.
- Explicitly prohibit shadows, gradients, floor planes, reflections, texture, and lighting variation in the chroma-key background.
- Ask for crisp edges, generous padding, and no use of the key color inside the subject.
- After chroma-key generation, remove the background locally with `python "/Users/eliot/code/yb/niche/ink-imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
## Ink relay CLI execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are ink relay CLI execution controls.
- See `references/cli.md` and `references/image-api.md` for those controls.
- In ink relay CLI mode, `gpt-image-2` is the default. It supports `quality=low|medium|high|auto`; use `low` for fast drafts and thumbnails, and move to `medium`, `high`, or `auto` for final assets.
- `gpt-image-2` always uses high fidelity for image inputs, so do not set `input_fidelity` with that model.
- If a transparent request needs true transparency, use `gpt-image-1.5` when the relay supports it because `gpt-image-2` does not support `background=transparent`.
...
- infographic-diagram: Define the audience and layout flow; label parts explicitly; require verbatim text; prefer higher quality in ink relay CLI mode for dense labels.
