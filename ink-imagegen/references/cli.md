...
`generate-batch` is the ink relay CLI path for many distinct prompts or deliverable assets.

## What this CLI does
- `generate`: generate a new image from a prompt
- `edit`: edit one or more existing images
- `generate-batch`: run many generation jobs from a JSONL file through the ink relay CLI

Real API calls require network access plus an API key and relay base URL. By default, the CLI reads them from Codex config: `~/.codex/auth.json` `OPENAI_API_KEY` and `~/.codex/config.toml` `model_providers.<model_provider>.base_url`. Environment variables override those defaults. `--dry-run` does not require network.

## Quick start (works from any repo)
Set a stable path to the skill CLI:

```
export IMAGE_GEN="/Users/eliot/code/yb/niche/ink-imagegen/scripts/image_gen.py"
```

Optional overrides:

```bash
export RELAY_IMAGE_BASE_URL="https://your-relay.example.com/v1"
...
Generate (requires Codex config or environment credentials + network):
...
- Do not silently switch from `gpt-image-2` to `gpt-image-1.5`; ask first unless true transparent output requires it or the user explicitly requested that model.
...
`gpt-image-2` is the default model for new ink relay CLI work.
...
- Do not use `--background transparent` with `gpt-image-2`; use `gpt-image-1.5` for model-native transparent output when the relay supports it, or fall back to chroma-key generation plus local removal.
...
True transparent output:

Use this when true model-native transparency is required and the relay supports `gpt-image-1.5` transparent output.
...
When using this path, explain briefly that `gpt-image-2` does not support `background=transparent`, so `gpt-image-1.5` is required for model-native transparency.

## Quality, input fidelity, and masks (ink relay CLI only)
These are explicit ink relay CLI controls.
...
## See also
- API parameter quick reference for ink relay CLI mode: `references/image-api.md`
- Prompt examples shared across both top-level modes: `references/sample-prompts.md`
- Network/sandbox notes for ink relay CLI mode: `references/codex-network.md`
- Transparent image workflow: `SKILL.md` and `/Users/eliot/code/yb/niche/ink-imagegen/scripts/remove_chroma_key.py`
